...
public class Picture extends SimplePicture 
{
  ...
  public void mirrorArms()
  {
      ...
      for(int row = 159; row < 190; row++)
      {
          ...
          {
              topPixel = pixels[row][col];
              bottomPixel = pixels[195 + (190 - row)][col];
              bottomPixel.setColor(topPixel.getColor());
          }
      }
  ...
  public void mirrorGull()
  {
    ...
    for (int row = 0; row < pixels.length; row++)
    {
      for (int col = 100; col < 367; col++)
      {
        leftPixel = pixels[row][col];
        rightPixel = pixels[row][380 + (367 - col)];
        rightPixel.setColor(leftPixel.getColor());
      }
    } 
  }
  ...
  public void copy(Picture fromPic, int startRow, int startCol, int beginRow, int beginCol, int endRow, int endCol)
  {
    ...
    Pixel[][] toPixels = this.getPixels2D();
    Pixel[][] fromPixels = fromPic.getPixels2D();
    for (int fromRow = beginRow, toRow = startRow; 
         fromRow < endRow &&
         toRow < toPixels.length; 
         fromRow++, toRow++)
    {
      for (int fromCol = beginCol, toCol = startCol; 
           fromCol < endCol &&
           toCol < toPixels[0].length;  
           fromCol++, toCol++)
      {
  ...
  {
      Picture one = new Picture("flower1.jpg");
      Picture two = new Picture("flower2.jpg");
      Picture three = new Picture("robot.jpg");
      
      one.mirrorVerticalRightToLeft();
      two.zeroBlue();
      three.mirrorHorizontal();
      three.fixUnderwater();
      
      this.copy(one, 0, 0, 25, 0, 45, 80);
      this.copy(two, 25, 0, 0, 0, 90, 50);
      this.copy(three, 85, 50, 0 , 0, 70, 30);
